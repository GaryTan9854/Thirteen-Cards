...
backend/
  ...
  game/
    ...
    rules.py       # ★ table(players) ContextVar ＋ cat_strength()：牌型順序隨人數（5/6 人桌同花 > 葫蘆）
  ml/              # ScoringNet (93-dim → μ/σ)
  tests/           # pytest（49 綠）—— 計分/牌型/牌組/降級/房間座位
...
## 測試
```bash
cd backend && ./venv_train/bin/python -m pytest tests -q     # 49 綠
```
...
## 5 / 6 人玩法（2026-09-16，決策委員會裁決・主委 Jack）

**裁決原文要點**：(0) 5 人加一副、6 人加兩副花色；(1) 藍桃算**黑國**、橘心算**紅國**，
全黑/全紅類報到照樣成立；(2) **鋼支 > 同花順**（6 人桌機率上同花順更少，但維持）；
(3) **5/6 人桌 鐵支 > 同花 > 葫蘆**（6 人桌鐵支與同花實測幾乎一樣，同花則明確比葫蘆稀有）；
(4) 鋼支計分與打槍 ×(N+1) 如前。說明頁「543賽制」分頁與「歷次委員會修訂記錄」都已補上。

| 人數 | 牌組 | 花色 |
|---|---|---|
| 4 | 52 張 | C D H S |
| 5 | 65 張 | ＋ **X**＝第二副黑桃（畫成**藍色** ♠） |
| 6 | 78 張 | ＋ **Y**＝第二副紅心（畫成**橘色** ♥，愛馬仕橘 `#F37021`） |
...
- ★★ **配色同一天改了三次，每一次的理由都要留著**（`utils/suits.ts` 開頭有完整紀錄）：
  1. 橘 `orange-500` ＋ 紅 `red-600` → Gary 實測分不出：**明度幾乎相同（53% vs 51%）**，只剩色相差。
  2. 綠 `emerald-600`（四色牌慣例）→ 分得開，但委員會裁定第二副紅心要**算紅國**，綠色的心說不通。
  3. **現行：愛馬仕橘 `#F37021` ＋ 更深更純的紅 `#D00000`** → 色相**與明度（41% vs 54%）同時拉開**。
  教訓：兩個顏色要分得開，**明度差比色相差重要**；只調色相、不調深淺，小字級一定糊在一起。
  ⚠ 紅色一改，4 人桌的 ♥♦ 也一起變深——刻意的，同一種紅不能有兩個色號。
- ★★ **顯示字元與畫出來的字元是兩件事**：後端 `Card.show()` 送 **空心**的 ♤ / ♡
  ...
  畫出來變成「♠undefined」。
- ★ 顏色語意跟著母花色：藍桃算「黑國」、橘心算「紅國」（委員會裁決；全黑/全紅等報到用）。
...
- ★★ **5/6 人桌牌型順序：鐵支 > 同花 > 葫蘆**（4 人桌維持 葫蘆 > 同花）——**只有這一格隨人數變**。
  依據（實打 1000 局量的中/尾墩出現率，比「隨機 5 張」的組合數更貼近體感）：
  ...
  （4 人桌實測順序與傳統一字不差，證明量法沒跑偏。6 人桌同花 vs 鐵支 z≈0.6 分不出來，故維持鐵支大。）

  **實作：`game/rules.py`**
  - `table(players)` 是 **ContextVar**，`play_one_game` 與 `/api/game/arrange`、`/api/manual/*` 入口都包在裡面。
    ★★ **不能用模組全域變數**：單機走 threadpool、連線結算走 run_in_executor，
    4 人桌和 6 人桌可能**同時**在算分，全域變數會讓排序漏到別桌且不報錯（有 thread 測試守）。
  - `Hand5.score_hand()`：5/6 人桌同花底分 170（`FLUSH_BASE_5P6P`），壓過葫蘆最高 164、低於鐵支最低 688。
    **只改分數，不改 `handtype_val`**——5＝同花是牌型的身分，到處當 key 用。
  - `hand_lookup`：位階表的同花塊（1277 列）與葫蘆塊（156 列）**相鄰**，5/6 人桌做精確平移對調；開機時 assert 相鄰。
  - ★★ **要比「誰大」一律問 `cat_strength(ht)`，絕不拿 HandCat 數字直接比**。
    2026-09-16 踩過：`enumerate_arrangements` 有一行 `if mid_cat > bot_cat: continue`，
    「中葫蘆、尾同花」在 5/6 人桌合法卻在算分前就被剪掉——**AI 永遠選不到，但不報錯、不倒水、測試照綠**。
    是用「特意湊一手只有新排法才合理的牌」才抓出來的。修後 300 局實測：4 人只出現「中同花尾葫蘆」42 次、
    5/6 人只出現「中葫蘆尾同花」18/8 次，完全鏡像。
  - 前端 `ManualArrange` 呼叫 `score_rows`／`arrange_info` **一定要帶 `players`**：
    後端雖能從花色反推，但沒拿到藍桃的 5 人手牌（約 3%）會被當 4 人桌，合法排法被判倒水。
...
### 待辦
- [ ] **5/6 人桌的位階表百分位**：順序已照裁決對調（`hand_lookup._reorder`），但百分位仍是 52 張牌窮舉的；
      精確版要 `build_win_rates.py` 加花色數參數，產 `hand_ranks_5.db` / `_6.db`（含鋼支列）。攻擊門檻 `_ATK_RANK*` 也是 4 人桌校準的
