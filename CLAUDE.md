...
- ★ **配色取自四色牌（four-colour deck）慣例**（♠黑 ♥紅 ♦藍 ♣綠）：藍桃 `sky-600`、綠心 `emerald-600`。
  ⚠ 第二副紅心**一開始用橘色，2026-09-16 Gary 實測退回**——橘在色輪上貼著紅，
  牌面那個字級只剩明度差，桌上分不出來；綠與紅是對立色才一眼就開。
  （TunaPoker 第一版 `PlayingCard.tsx` 用的是深色桌面版的 `emerald-400`／`sky-400`；
  十三支是白底牌面，各取深一階。）
- ★★ **顯示字元與畫出來的字元是兩件事**：後端 `Card.show()` 送 **空心**的 ♤ / ♡
  ...
  畫出來變成「♠undefined」。
- ★ 顏色語意跟著母花色：藍桃仍算「黑」、綠心仍算「紅」（全黑/全紅等報到用）。
