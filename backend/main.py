...
APP_VERSION = "2.10.16"
APP_BUILD = "366"  # deploy.sh 自動寫入（= git commit 總數）
...
def arrange_hand(req: ArrangeRequest):
    ...
    if strategy == "monte_carlo":
        ...
        # RuleAlpha3 ignores attitude (always 0); keep aliases for backward compat.
        from game.arrange import best_arrangement_rulealpha3
        result = best_arrangement_rulealpha3(req.hand, attitude=0.0)
        if result:
            h13.htop, h13.hmid, h13.hbot = result
            h13.ss = [h13.htop.score, h13.hmid.score, h13.hbot.score]
            arr = h13
            ...
            h13.arrange13()
            arr = h13
    elif strategy in ("rulealpha4", "rulealpha4_aggressive", "rulealpha4_conservative"):
        from game.arrange import best_arrangement_rulealpha4
        attitude = {"rulealpha4_aggressive": 0.8, "rulealpha4_conservative": -0.8}.get(strategy, 0.0)
        result = best_arrangement_rulealpha4(req.hand, attitude=attitude)
        if result:
            h13.htop, h13.hmid, h13.hbot = result
            h13.ss = [h13.htop.score, h13.hmid.score, h13.hbot.score]
            arr = h13
...
def manual_arrange_info(req: ManualInfoRequest):
    ...
    def _is_monster(row: str, position: str) -> bool:
        ...
        if position == 'mid': return row in _MID_MONSTER
        return row in _BOT_MONSTER

    active_heuristic = [g for g in groups if not g.get("dominated") and g.get("variants")]
    for gi in active_heuristic:
        if gi.get("dominated"):
            continue
        vi = gi["variants"][0]
        ti, mi, bi = vi["top_type"], vi["mid_type"], vi["bot_type"]
        ci_top = _TOP_CAT.get(ti, 0)
        ci_mid = _CAT.get(mi, 0)
        ci_bot = min(_CAT.get(bi, 0), 8)

        for gj in active_heuristic:
            if gj is gi or gj.get("dominated") or not gj.get("variants"):
                continue
            vj = gj["variants"][0]
            tj, mj, bj = vj["top_type"], vj["mid_type"], vj["bot_type"]
            cj_top = _TOP_CAT.get(tj, 0)
            ...
            if gi_wins == 1 and not gj_pile_is_monster:
                gi_monster = (
                    (w_top and _is_monster(ti, 'top')) or
                    (w_mid and _is_monster(mi, 'mid')) or
                    (w_bot and _is_monster(bi, 'bot'))
                )
                if gi_monster:
                    gj["dominated"] = True
